...
quaterntions2 = []
timestamp2 = []
data = np.loadtxt(filepath + '/imu_int_pose0.txt')
# timestamp2 = data[:,0]
...
ax.plot(position1[:,0], position1[:,1], position1[:,2], label='middle method')
ax.plot(position2[:,0], position2[:,1], position2[:,2], label='euler method')
ax.plot([position[0,0]], [position[0,1]], [position[0,2]], 'r.', label='start')

### plot 2d
#ax.plot(position[:,0], position[:,1],0*position[:,2], label='gt')
#ax.plot(position1[:,0], position1[:,1],0*position1[:,2], label='middle method')
#ax.plot(position2[:,0], position2[:,1],0*position2[:,2], label='euler method')
#ax.plot([position[0,0]], [position[0,1]], [0], 'r.', label='start')
